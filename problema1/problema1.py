...
Original file is located at
    https://colab.research.google.com/drive/1MDgk18i2n1wduLjC1gLrcSkMwOBC9E4I
"""
...
t=0
ma=np.genfromtxt("masas.txt") #masa
rx=np.genfromtxt("distancias.txt") #distancia al sol
ry=np.zeros(m)
vx=np.zeros(m)
vy=np.genfromtxt("velocidad.txt") #velocidad
...
pt=np.genfromtxt("periodos.txt")*365
ps=np.zeros(m-1)
psuma=np.zeros(m-1)
cont=np.zeros(m-1)
rya=np.zeros(m)

#inicializar variables para conservación energia
ec0=0
...
f2 = open("conservacion_energia.txt","w")

#calcular la energía cinéctica total inicial
for i in range(m):
  ec0=ec0+0.5*ma[i]*vy[i]*vy[i]
...
for k in range(n):
  rya[3]=ry[3] #calculamos primero la posicion de la tierra para geocentrismo
  rx[3]=rx[3]+h*vx[3]+(h*h*0.5)*ax[3]
  ry[3]=ry[3]+h*vy[3]+(h*h*0.5)*ay[3]
  wx[3]=vx[3]+(h*0.5)*ax[3]
  wy[3]=vy[3]+(h*0.5)*ay[3]
  for i in range(m):
    if (i!=3):
      rya[i]=ry[i]
    ...
    if (ref=="g"):
      print(rx[i]-rx[3], ",",ry[i]-ry[3], file=f) #escribe las posiciones en un archivo
    else:
      print(rx[i], ",",ry[i], file=f)
  ...
  #comprobacio conservacion energia
  ec=0
  for i in range(m):
    ec=ec+0.5*ma[i]*(vx[i]*vx[i]+vy[i]*vy[i])
  print("Ec_0=",ec0,"Ec=", ec, "Proporción= ", ec/ec0,file=f2)
